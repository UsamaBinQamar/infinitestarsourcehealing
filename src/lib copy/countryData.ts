...
export const countries: CountryData[] = [
  ...
  { code: "CA", name: "Canada", dialCode: "1_CA", flag: "🇨🇦" }, // Using "1_CA" to differentiate from US
  ...
  { code: "DO", name: "Dominican Republic", dialCode: "1_DO", flag: "🇩🇴" }, // Using "1_DO" to differentiate from US
...
// List of countries from ClickSend's service (most commonly used)
...
// Use this function to format a phone number with its country code
export function formatPhoneNumber(countryCode: string, phoneNumber: string): string {
  // Remove any non-digit characters from the phone number
  const cleanNumber = phoneNumber.replace(/\D/g, '');
  
  // Handle special cases for North American codes that we've modified with suffixes
  if (countryCode === "1_CA" || countryCode === "1_DO") {
    return `+1${cleanNumber}`; // Just use plain +1 for all North American numbers
  }
...
// Function to get the flag emoji for a country by its dial code
export function getFlagByDialCode(dialCode: string): string {
  // Special case for +1 (North America) - prioritize USA 
  if (dialCode === "1") {
    return "🇺🇸"; // Always return USA flag for +1
  }
